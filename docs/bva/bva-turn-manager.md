...
| Test Case 8 | input1 = valid card ("shuffle"), input2 = SHUFFLE, input3 = true (can play). Input 4 = N/A. Make input 5 false so it prompts again. input1 = valid card ("targeted attack"), input2 = TARGETED_ATTACK, input3 = true (can play). Input 4 = N/A. Make input 5 true so it doesn't prompt again. | Calls `doShuffle()` and continues loop. Calls `doTargetedAttack()` and ends loop | no           |
