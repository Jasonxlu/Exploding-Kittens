...
| Test Case 7 | input1 = valid card ("alter the future"), input2 = ALTER_THE_FUTURE, input3 = true (can play). Input 4 = N/A. Make input 5 false so it prompts again. input1 = valid card ("skip"), input2 = SKIP, input3 = true (can play). Input 4 = N/A. Make input 5 true so it doesn't prompt again.     | Calls `doAlterTheFuture()` and continues loop. Calls `doSkip()` and ends loop    | no           |
